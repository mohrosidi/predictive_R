...
Terdapat beberapa paket yang digunakan dalam pembuatan model prediktif menggunakan *naive bayes*. Paket-paket yang digunakan ditampilkan sebagai berikut:
...
8. `MLmetrics` : paket pembantu dalam perhitungan metrik klasifikasi multi kelas
...
Proses *training* berlangsung selama 178.379 detik dengan 72 buah model yang terbentuk. Model terbaik dipilih berdasarkan nilai **AUC** terbesar. Berdasarkan kriteria tersebut model yang terpilih adalalah model yang memiliki nilai `ft` = `r model_fit_cv$bestTune %>% dplyr::select(fL) %>% pull()`, `usekernel` = `r model_fit_cv$bestTune %>% dplyr::select(usekernel) %>% pull()` dan `adjust` = `r model_fit_cv$bestTune %>% dplyr::select(adjust) %>% pull()`. Nilai **AUC** rata-rata model terbaik adalah sebagai berikut:
