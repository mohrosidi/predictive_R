...
Analisis komponen utama dapat digunakan untuk mewakili variabel berkorelasi dengan sejumlah kecil fitur tidak berkorelasi (disebut komponen prinsip) dan komponen yang dihasilkan dapat digunakan sebagai prediktor dalam model regresi linier. Proses dua langkah ini dikenal sebagai *Principal Components Regression* (PCR) (Massy 1965) dan diilustrasikan dalam gambar di bawah ini.
...
Proses *training* berlangsung selama 16.579 detik. Model terbaik dipilih berdasarkan nilai **RMSE** terbesar. Berdasarkan kriteria tersebut model yang terpilih adalalah model yang memiliki nilai `ncomp` = `r pc_fit_cv$bestTune$ncomp`. Nilai **RMSE** rata-rata model terbaik adalah sebagai berikut:
...
Berdasarkan hasil yang diperoleh, nilai **RMSE** rata-rata model sebesar `r pc_rmse`.
